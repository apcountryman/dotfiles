...
function install_dotfiles()
{
    ...
    local dotfiles; mapfile -t dotfiles < <( git -C "$repository" ls-files 'dotfiles/' | cut -d / -f 2 | sort -u ); readonly dotfiles

    local dotfile
    for dotfile in "${dotfiles[@]}"; do
        local installed_dotfile="$HOME/$dotfile"
        ...
        if [[ -e "$installed_dotfile" ]]; then
            local archived_dotfile="$archive/$dotfile"

            if [[ -e "$archived_dotfile" ]]; then
                abort "'$dotfile' archive already exists"
            fi
